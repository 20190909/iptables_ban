...
Update_Shell(){
	sh_new_ver=$(wget --no-check-certificate -qO- -t1 -T3 "https://raw.githubusercontent.com/Deinococci/main/iptables_ban.sh"|grep 'sh_ver="'|awk -F "=" '{print $NF}'|sed 's/\"//g'|head -1)
	[[ -z ${sh_new_ver} ]] && echo -e "${Error} Unable to connect to Github!" && exit 0
	wget -N --no-check-certificate "https://raw.githubusercontent.com/Deinococci/main/iptables_ban.sh" && chmod +x iptables_ban.sh
